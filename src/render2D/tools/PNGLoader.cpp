...
{
    cerr << "PNG lib warning : " << warning_msg << std::endl;
}
...
Texture* PNGLoader::ReadPNGFromFile (const string& filename, GLint filter, GLint wrap)
{
	Texture* texture;
    ...
    try
    {
        ...
        switch(color)
        {
          ...
          case(PNG_COLOR_TYPE_RGB): pixelFormat = GL_RGB; internalFormat = 3; break;
          case(PNG_COLOR_TYPE_PALETTE): pixelFormat = GL_RGB; internalFormat = 3; break;
          case(PNG_COLOR_TYPE_RGB_ALPHA): pixelFormat = GL_RGBA; internalFormat = 4; break;
          default: break;                
        }
		...
		texture = new Texture(width, height, pixelFormat, internalFormat, dataType, data);
    }
    catch (int e)
    {
		cerr << "[PNGLoader] Incorrect PNG format for " << filename << " : " << e <<  endl;
        texture = new Texture();
    } 
	fin.close();
